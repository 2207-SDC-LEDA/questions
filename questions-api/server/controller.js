...
const getQuestions = (req, res) =>{
  ...
  let pidkey = pid.toString()+'p';
  let isCached = false;

  redisClient.get(pidkey)
    .then((response) =>{
      // console.log('parameter', req.query);
      let sendback = {};
      ...
      if(response) {
        console.log("cached");
        sendback['result']=JSON.parse(response);
        res.send(sendback);
      } else {
...
const getAnswers =(req, res) =>{
  ...
  let qidkey = qid.toString()+'q';
  let isCached = false;

  redisClient.get(qidkey)
  .then((response) =>{
    // console.log('parameter', req.query);
    let resa = {};
    resa.question = qid;
    if(response) {
      console.log("cached");
      resa.results = JSON.parse(response);
      res.send(resa);
    } else {
      console.log(" not cached");
